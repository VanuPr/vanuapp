...
interface UplineCoordinator {
    role: 'Admin' | 'state' | 'district' | 'block' | 'panchayat';
    state?: string;
    district?: string;
    blockName?: string;
    name?: string;
...
const terms = {
    state: {
        en: [
            ...
            "Upon submission of the registration fee, you will receive an ID card and an offer letter.",
            "As a State Coordinator, it is mandatory to appoint one District Coordinator in every district of the state you are working in.",
            "After the opening of ten districts in your state, a state office will be provided to you.",
            "After the office opens, your basic salary will be ₹50,000 + ₹10,000 TA/DA.",
            "From the joining of all districts in your state, you will receive ₹200 per joining, ₹2 per Kisan Card, and ₹5 per child registration.",
            "It is mandatory to deposit the daily collection from your state into the company account on the same day.",
            "If you withhold any collection, action will be taken against you, and you will be barred from working in your state.",
            "It is mandatory to keep all employees of your state's districts under your discipline.",
            "It is mandatory to get stock points opened in all districts of your state.",
            "In case of any dispute, it will be resolved in the Godda court with the cooperation of the Godda Head Branch.",
            "In the event of any mishap during work, your family will be provided with an insurance of five lakh rupees by the company. And in case of any accident, up to one lakh rupees will be given for treatment expenses."
        ],
        hi: [
            "आपका रजिस्ट्रेशन शुल्क 2550/- रुपया है।",
            "रजिस्ट्रेशन शुल्क जमा करने पर आपको आय डी कार्ड तथा ऑफर लेटर दिया जायेगा।",
            "स्टेट कॉर्डिनेटर का कार्य, आप जिस स्टेट में काम कर रहे उस स्टेट के सभी जिले में एक एक जिला कॉर्डिनेटर कि नियुक्ति करना अनिवार्य होगा।",
            "अपने स्टेट के दस जिले का ओपनिंग बाद आपके स्टेट में एक स्टेट ऑफिस दिया जायेगा।",
            "ऑफिस खुलने के बाद आपकी बेसिक तनख्वा 50000+10000 TA+DA दिया जाता है।",
            "आपके स्टेट के सभी जिले का जोइनिंग से प्रति जोइनिंग दो सौ रुपया, प्रति किसान कार्ड दो रुपया तथा प्रति बच्चा रजिस्ट्रेशन पांच रुपया दिया जायेगा।",
            "अपने स्टेट में जो भी रोजाना का कलेक्शन आता है उसे उसी दिन कंपनी में लगाना अनिवार्य है।",
            "कोई भी कलेक्शन अगर आप रोकते है तो आपके ऊपर कार्यवाही कि जाएगी तथा आपको अपने स्टेट में कार्य करने से रोक दिया जायेगा।",
            "अपने स्टेट के सभी जिले के सभी एम्पलॉय को अपने अनुशासन में रखना अनिवार्य होगा।",
            "अपने स्टेट के सभी जिले में स्टॉक पॉइंट खुलवाना अनिवार्य होगा।",
            "बर्तमान अवस्था में कोई भी विबाद होने पर उसका निराकरण गोड्डा के हेड ब्रांच के सहयोग के साथ गोड्डा न्यायलय में किया जायेगा।",
            "कार्य करने के दौरान कोई भी अनहोनी होने पर कंपनी द्वारा आपके परिवार को पांच लाख का बीमा दी जाएगी. और कोई भी दुर्घटना होने पर एक लाख तक का इलाज हेतु खर्च दिया जायेगा।"
        ]
    },
    district: {
        en: [
...
export default function EmployeeRegistrationPage() {
    ...
    useEffect(() => {
        if(uplineCoordinator) {
            ...
            if (uplineCoordinator.role === 'state') {
                initialPosition = 'district';
            } else if (uplineCoordinator.role === 'block') {
    ...
    const checkLocationAvailability = async (): Promise<boolean> => {
        ...
        if (formData.positionType === 'state') {
            if (querySnapshot.size >= 5) {
                 toast({
                    variant: 'destructive',
                    title: 'Position Filled',
                    description: `The maximum number of State Coordinators (5) for ${formData.preferredLocation} has been reached.`
                });
                return false;
            }
        } else {
    ...
    const disableStatePosition = uplineCoordinator?.role === 'state' || uplineCoordinator?.role === 'district' || uplineCoordinator?.role === 'block' || uplineCoordinator?.role === 'panchayat';
    ...
    const isStateDisabled = uplineCoordinator?.role === 'state' || uplineCoordinator?.role === 'district' || uplineCoordinator?.role === 'block' || uplineCoordinator?.role === 'panchayat';
    ...
    useEffect(() => {
        ...
        } else if (formData.positionType === 'state') {
            location = formData.state;
        }
        setFormData(prev => ({ ...prev, preferredLocation: location }));
    }, [formData.panchayat, formData.blockName, formData.district, formData.state, formData.positionType]);
      ...
      <main className="flex-1 py-16 md:py-24">
        <div className="container mx-auto px-4 md:px-6">
          <div className="text-center mb-12">
            ...
            <p className="mt-4 max-w-3xl mx-auto text-muted-foreground">Please fill the form in CAPITAL LETTERS only. Fields marked with <span className="text-destructive">*</span> are required.</p>
          </div>
          ...
          <form ref={formRef} onSubmit={handleContinue} className="max-w-4xl mx-auto space-y-8">
            <Card>
                ...
                <CardContent>
                     <RadioGroup onValueChange={(v) => handleRadioChange('positionType', v)} value={formData.positionType} className="grid md:grid-cols-4 gap-4" required>
                        <div>
                            <RadioGroupItem value="state" id="pos_state" className="peer sr-only" disabled={disableStatePosition} />
                            <Label htmlFor="pos_state" className="flex flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary peer-disabled:opacity-50 peer-disabled:cursor-not-allowed">State Coordinator</Label>
                        </div>
                        <div>
     ...
     <Dialog open={isPaymentModalOpen} onOpenChange={setIsPaymentModalOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Please choose how you would like to pay the application fee.
                </DialogDescription>
            </DialogHeader>
     ...
     <Dialog open={isQrDialogOpen} onOpenChange={setIsQrDialogOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Please scan the QR code to pay the fee. Once paid, confirm the transaction.
                </DialogDescription>
            </DialogHeader>
